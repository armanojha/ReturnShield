    ...
    <div className="return-form">
      <form onSubmit={handleSubmit} style={{ maxWidth: '500px' }}>
        <div style={{ marginBottom: 'var(--space-3)' }}>
          <label style={{ display: 'block', marginBottom: 'var(--space-1)', fontWeight: 600 }}>Order ID</label>
          <input
            type="text"
            value={form.order_id}
        ...
        <div style={{ marginBottom: 'var(--space-3)' }}>
          <label style={{ display: 'block', marginBottom: 'var(--space-1)', fontWeight: 600 }}>Return Reason</label>
          <select
            value={form.reason}
        ...
        <div style={{ marginBottom: 'var(--space-3)' }}>
          <label style={{ display: 'block', marginBottom: 'var(--space-1)', fontWeight: 600 }}>Evidence (optional)</label>
          <textarea
      ...
      {result && (
        <div style={{ marginTop: 'var(--space-4)', padding: 'var(--space-4)', background: 'var(--surface-raised)', borderRadius: 'var(--radius)' }}>
          <h3 style={{ marginBottom: 'var(--space-2)' }}>Return Request Submitted</h3>
          <p>Case ID: <strong>{result.case?.case_id}</strong></p>
          <p>Status: <Badge kind={result.case?.status}>{result.case?.status}</Badge></p>
          {result.replayed && <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>This request was already submitted (idempotent replay).</p>}
        </div>
      )}
    </div>
  );
