...
interface TimelineEvent {
  ...
  case_id: string;
  timestamp: string;
  type: 'RETURN_RECEIVED' | 'POLICY_DECIDED' | 'REVIEW_REQUESTED' | 'EXPLANATION_AVAILABLE' | 'REVIEWER_DECISION';
  actor_id: string;
  message: string;
}

/** Vertical timeline of case events. */
export const Timeline: FC<PropsWithChildren<{
  ...
  return (
    <div className="timeline">
      {events.map((event, idx) => (
        <div key={event.event_id} className="timeline__item" style={{ position: 'relative', paddingLeft: 'var(--space-6)', marginBottom: 'var(--space-4)' }}>
          <div style={{ position: 'absolute', left: 0, top: 0, width: '12px', height: '12px', borderRadius: '50%', backgroundColor: typeColors[event.type] || 'var(--text)', border: '2px solid var(--surface)' }} />
